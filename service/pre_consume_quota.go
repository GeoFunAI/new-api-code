...
func ReturnPreConsumedQuota(c *gin.Context, relayInfo *relaycommon.RelayInfo) {
	if relayInfo.FinalPreConsumedQuota != 0 {
		logger.LogInfo(c, fmt.Sprintf("用户 %d 请求失败, 返还预扣费%s %s", relayInfo.UserId, logger.GetQuotaUnit(), logger.FormatQuota(relayInfo.FinalPreConsumedQuota)))
		gopool.Go(func() {
...
func PreConsumeQuota(c *gin.Context, preConsumedQuota int, relayInfo *relaycommon.RelayInfo) *types.NewAPIError {
	...
	if userQuota > trustQuota {
		...
		if !relayInfo.TokenUnlimited {
			...
			if tokenQuota > trustQuota {
				// 令牌额度充足，信任令牌
				preConsumedQuota = 0
				logger.LogInfo(c, fmt.Sprintf("用户 %d 剩余%s %s 且令牌 %d %s %d 充足, 信任且不需要预扣费", relayInfo.UserId, logger.GetQuotaUnit(), logger.FormatQuota(userQuota), relayInfo.TokenId, logger.GetQuotaUnit(), tokenQuota))
			}
		} else {
			// in this case, we do not pre-consume quota
			// because the user has enough quota
			preConsumedQuota = 0
			logger.LogInfo(c, fmt.Sprintf("用户 %d %s充足且为无限%s令牌, 信任且不需要预扣费", relayInfo.UserId, logger.GetQuotaUnit(), logger.GetQuotaUnit()))
		}
	}
	...
	if preConsumedQuota > 0 {
		...
		logger.LogInfo(c, fmt.Sprintf("用户 %d 预扣费 %s, 预扣费后剩余%s: %s", relayInfo.UserId, logger.FormatQuota(preConsumedQuota), logger.GetQuotaUnit(), logger.FormatQuota(userQuota-preConsumedQuota)))
	}
